...
router.get('/', function(req, res, next) {
    let sql = 'SELECT * FROM poslodavac_informacije'
...
router.post('/dodajPoslodavca', function(req, res, next) {
    ...
    if(err)
    {
        ...
        let sql = 'INSERT INTO `poslodavac_informacije`( `ime_firme`, `pretplacen_do`, `moze_da_koristi`) VALUES (?,?,?)';
        let vrednosti = [ime_firme,pretplacen_do,moze_da_koristi];
...
router.post('/izmeniPoslodavca', function(req, res, next) {
    ...
    con.query(sql,[ime,prezime,email,lozinka,rola,ime_firme,ime_firme_staro], function(err,result){
    if(err)
    {
        console.log(err);
        res.status(500);
        ...
        var moze_da_koristi=req.body.moze_da_koristi;
        let ime_firme_staro=req.body.ime_firme_staro;
        let sql = 'UPDATE `poslodavac_informacije` SET `ime_firme`=? , `pretplacen_do`=? ,`moze_da_koristi`=? WHERE `ime_firme`=?';
        let vrednosti = [ime_firme,pretplacen_do,moze_da_koristi,ime_firme_staro];
        con.query(sql,vrednosti, function(err,result){
            if(err)
            {
                res.status(500);
...
router.post('/obrisiPoslodavca', function(req, res, next) {
    ...
    let sql = 'DELETE FROM `korisnici` WHERE ime_firme=?';
...
router.post('/ucitajPodatkeOPoslodavcu', function(req, res, next) {
    ...
    var ime_firme=req.body.ime_firme;
    let sql = 'SELECT k.ime,k.prezime,k.email,k.lozinka,k.ime_firme,p.pretplacen_do,p.moze_da_koristi FROM korisnici k,poslodavac_informacije p WHERE k.ime_firme=? AND  k.ime_firme=p.ime_firme';
